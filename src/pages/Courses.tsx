      ...
      <div className="page-container">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {courseCategories.map((category) => (
            <div key={category.name} className="glass-card p-4 text-center hover:border-amber-500/30 transition-all cursor-pointer">
              ...
              <p className="text-xs text-gray-400 mt-1">{category.count} courses</p>
            </div>
          ))}
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredCourses.map((course) => (
              <div key={course.title} className="glass-card overflow-hidden">
                <div className="h-48 bg-gradient-to-br from-amber-900/30 to-gray-900/30 relative">
                  {course.featured && (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="bg-white/10 backdrop-blur-sm p-4 rounded-full">
                      <course.icon className="h-12 w-12 text-amber-400" />
                    </div>
                  </div>
                </div>
        ...
        {/* Exam Prep Section */}
        <div className="mb-16">
          <h2 className="text-2xl font-bold mb-6 text-white">Exam Preparation</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {examPrep.map((exam) => (
              <div key={exam.title} className="glass-card p-6 flex flex-col md:flex-row gap-6">
                <div className="md:w-1/3">
                  <div className="h-full bg-gradient-to-br from-amber-900/30 to-gray-900/30 rounded-lg flex items-center justify-center p-6">
                    <exam.icon className="h-16 w-16 text-amber-400" />
                  </div>
                </div>
                
                <div className="md:w-2/3">
                  <h3 className="font-bold text-xl text-white mb-2">{exam.title}</h3>
        ...
        <div className="glass-card p-8 mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {certificateCourses.map((course) => (
              <div key={course.title} className="bg-amber-900/10 p-4 rounded-lg">
                <div className="font-bold text-white mb-1">{course.title}</div>
                <div className="text-xs text-gray-400 mb-3">Duration: {course.duration}</div>
